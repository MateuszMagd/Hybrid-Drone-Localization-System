...
from Utils.imageshow import displey_two_images, add_red_dot_to_image
import pandas
import random
import math
...
def calculate_random_offset(lat, lon, zoom=17, max_distance=None):
    ...
    # 1 stopień szerokości geograficznej to około 111000 m
    degree_to_meter = 111000
    
    # Przemieszczenie w stopniach, które odpowiada odległości w metrach
    max_lat_offset = max_distance / degree_to_meter  # Maksymalna zmiana szerokości geograficznej
    max_lon_offset = max_distance / (degree_to_meter * math.cos(math.radians(lat)))  # Korekta dla długości geograficznej
    
    # Losowanie kierunku (może być na północ, południe, wschód, zachód)
    lat_offset = random.uniform(-max_lat_offset, max_lat_offset)  # Losowa zmiana szerokości
    lon_offset = random.uniform(-max_lon_offset, max_lon_offset)  # Losowa zmiana długości
    
    # Nowe współrzędne po dodaniu losowego offsetu
